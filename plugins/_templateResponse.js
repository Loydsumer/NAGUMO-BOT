...
export async function all(m, chatUpdate) {
  ...
  if (m.message.buttonsResponseMessage) {
    ...
    try {
      ...
      id = m.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson;
    }
  }
  ...
  for (const name in global.plugins) {
    ...
    // تأكد من وجود صلاحيات الأدمن إذا كانت مطلوبة
    ...
    if (match && (usedPrefix = (match[0] || '')[0])) {
      ...
      let [command] = noPrefix.trim().split(/\s+/).filter((v) => v);
