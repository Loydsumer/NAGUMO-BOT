...
const handler = async (m, { conn, args, command }) => {
  ...
  if (!args.length) {
    return m.reply(`🧞‍♀️ *طريقة استخدام الأمر (${command})*:
...
ＯᏀᎯᎷᏆ 👻 ᏴᎾᎢ`);
  ...
  if (!res || !res.subtitle) return m.reply('🧞‍♀️ لم يتم العثور على ترجمة لهذا الفيديو.');

  const translated = await translate(res.subtitle, 'ar');

  let teks = `🧞‍♀️ *استوحى من فيديو يوتيوب*\n\n${translated[0]}\n\nＯᏀᎯᎷᏆ 👻 ᏴᎾᎢ`;

  await conn.sendMessage(m.chat, {
    text: teks,
    contextInfo: {
      externalAdReply: {
        title: "🧞‍♀️ استوحى من فيديو",
