...
let handler = async (m, { conn, text }) => {
    ...
    if (!/image/.test(mime)) throw `أين الصورة؟ 🧞‍♀️`
    
    try { q = m.quoted.download() }
    catch (e) { q = m.download() }

    m.reply(`🧞‍♀️ جاري إنشاء الفيديو... انتظر قليلًا.`)

    running(await q).then(vid => conn.sendFile(m.chat, vid, 'run.mp4', `🧞‍♀️ تم الإنشاء بنجاح!`, m))
}
...
function running(img, duration = 10, fps = 60) {
    return new Promise((resolve, reject) => {
        ...
        console.log(`🧞‍♀️ تم حفظ الصورة المؤقتة:`, img)
        ...
        console.log(`🧞‍♀️ تشغيل FFmpeg:`, 'ffmpeg', ...args)
        ...
        spawn('ffmpeg', args, { stdio: 'inherit' })
            ...
            .on('close', () => {
                try {
                    ...
                    fs.unlinkSync(o)
                    console.log(`🧞‍♀️ تم حذف الملفات المؤقتة.`)
                } catch (e) {
                    reject(e)
                }
            })
