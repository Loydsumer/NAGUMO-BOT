...
let handler = async function (m, { conn, text, usedPrefix, command }) {
  ...
  if (user.registered) {
    let msgTxt = `🚫 أنت مسجل بالفعل.\n⛩️ اضغط على الزر لحذف التسجيل 🧞‍♀️`
...
╰──────────────────────────╯`
  ...
  if (!Reg.test(text)) {
    let fixedAge = 20
    return m.reply(`❌ الصيغة غير صحيحة.\nاكتب: *${usedPrefix + command} الاسم.العمر*\nمثال: *${usedPrefix + command} ${nameFromContact}.${fixedAge}* 🧞‍♀️`)
  }

  let [_, namex, splitter, agex] = text.match(Reg)
  if (!namex || !agex) return m.reply('❌ تأكد من كتابة الاسم والعمر 🧞‍♀️')
  if (namex.length >= 100) return m.reply('❌ الاسم طويل جدًا 🧞‍♀️')
  let edad = parseInt(agex)
  if (isNaN(edad)) return m.reply('❌ العمر غير صالح 🧞‍♀️')
  if (edad > 1000) return m.reply('❌ العمر غير منطقي 🧞‍♀️')
  if (edad < 5) return m.reply('❌ العمر صغير جدًا 🧞‍♀️')
