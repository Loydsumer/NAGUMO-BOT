...
async function searchTrack(query) {
  ...
  if (!track) throw '🧞‍♀️ لم يتم العثور على الأغنية';
  return track;
}
...
async function fetchAlbum(id) {
  ...
  if (!album?.tracks?.items?.length) throw '🧞‍♀️ الألبوم فارغ أو غير متاح';
  return album;
}
...
async function fetchDownload(trackId, quality) {
  ...
  if (!url) throw '🧞‍♀️ رابط التحميل غير متوفر';
  return url;
}
...
const handler = async (m, { text, args, conn, usedPrefix, command }) => {
  try {
    ...
    if (!text) {
      let helpMsg = `🧞‍♀️ *طريقة الاستخدام الصحيحة*\n\n`;
    ...
    if (text.includes('qobuz.com/album/')) {
      ...
      if (!id) throw '🧞‍♀️ رابط الألبوم غير صحيح';
    ...
    if (!q.find(q => q.id === quality)) {
      throw `🧞‍♀️ الجودة ${quality} غير متوفرة\nالجودات المتاحة: ${q.map(q => q.id).join(', ')}`;
    }

    await m.reply(`🧞‍♀️ جاري تحميل الألبوم: *${album.title}*...`);
