...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (/audio|video/.test(mime)) {
		...
		const nisa = async (audio) => {
			return new Promise(async (resolve, reject) => {
				...
				ws.onopen = function () {
					console.log("تم الاتصال بخادم WebSocket 🧞‍♀️");
				};
				...
				ws.onmessage = async function (event) {
					...
					switch (message.msg) {
						...
						case "send_data":
							console.log("جاري معالجة الصوت 🧞‍♀️...");
				...
				ws.onclose = function (event) {
					if (event.code === 1000) {
						console.log("تم اكتمال المعالجة 🧞‍♀️");
					} else {
						msg.reply("خطأ: فشل الاتصال بـ WebSocket 🧞‍♀️");
					}
					resolve(result);
				};
			});
	...
	} else throw `يرجى الرد على فيديو/صوت باستخدام الأمر *${usedPrefix + command}* 🧞‍♀️`;
};

handler.help = ["suarakobo *رد على فيديو/صوت* 🧞‍♀️", "صوتكوبو *رد على فيديو/صوت* 🧞‍♀️"];
handler.command = ["suarakobo", "صوتكوبو"];
handler.tags = ["ai"];
handler.limit = true;

handler.usage = {
    suarakobo: "الرد على فيديو/صوت لتحويله إلى صوت باستخدام خدمة الكوبو. مثال:\n  رد على صوت أو فيديو وأرسل الأمر *suarakobo* 🧞‍♀️",
    صوتكوبو: "الرد على فيديو/صوت لتحويله إلى صوت باستخدام خدمة الكوبو. مثال:\n  رد على صوت أو فيديو وأرسل الأمر *صوتكوبو* 🧞‍♀️"
};
