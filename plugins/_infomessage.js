...
const nombre = `> ❀ @${usuario.split('@')[0]} قام بتغيير *اسم المجموعة*.\n> ✦ الاسم الجديد:\n> *${m.messageStubParameters[0]}*.`

const foto = `> ❀ تم تغيير *صورة المجموعة*.\n> ✦ قام بالفعل:\n> » @${usuario.split('@')[0]}`

const edit = `> ❀ @${usuario.split('@')[0]} قام بتغيير إعدادات التعديل.\n> ✦ الآن يمكن لـ: ${m.messageStubParameters[0] == 'on' ? 'المشرفين فقط' : 'الجميع'} تعديل إعدادات المجموعة.`

const newlink = `> ❀ تم *إعادة تعيين رابط المجموعة*.\n> ✦ بواسطة:\n> » @${usuario.split('@')[0]}`

const status = `> ❀ تم *${m.messageStubParameters[0] == 'on' ? 'قفل' : 'فتح'}* المجموعة بواسطة @${usuario.split('@')[0]}\n> ✦ الآن يستطيع ${m.messageStubParameters[0] == 'on' ? 'المشرفون فقط' : 'الجميع'} إرسال الرسائل.`

const admingp = `> ❀ @${users.split('@')[0]} أصبح *مشرفًا*.\n> ✦ تمت الترقية بواسطة:\n> » @${usuario.split('@')[0]}`

const noadmingp = `> ❀ @${users.split('@')[0]} لم يعد مشرفًا.\n> ✦ تمت الإزالة بواسطة:\n> » @${usuario.split('@')[0]}`


/* حذف الملفات عند حدوث خطأ */
...
console.log(`${chalk.yellow.bold('✎ حذف!')} ${chalk.greenBright(`'${file}'`)}\n${chalk.redBright('لتجنب حدوث undefined في الدردشة.')}`)
}}} 
...
if (!metadata?.participants) throw new Error("لم يتم الحصول على المشاركين")
