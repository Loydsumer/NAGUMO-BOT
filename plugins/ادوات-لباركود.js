...
let handler = async (m, { conn, text, command }) => {
    ...
    if (!text) return conn.reply(m.chat, `🧞‍♀️ *طريقة الاستخدام:* \n- أرسل الأمر *\`.${commandText} <النص>\`*\n- سيتم تحويل النص إلى ${commandText} وإرساله إليك.\n\n🧞‍♀️ *مثال:* \n\`.${commandText} مرحبًا\``, m)

    conn.sendFile(m.chat, await toDataURL(text.slice(0, 2048), { scale: 8 }), 'qrcode.png', `🧞‍♀️ *تم إنشاء ${commandText} بنجاح!* \n\nＯᏀᎯᎷᏆ 👻 ᏴᎾᎢ`, m)
}
