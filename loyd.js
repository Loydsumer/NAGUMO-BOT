//هنا تشغيل تقدر تعدل برضو مش مكتمل🐦
...
global.authFile = `sessions`
  ...
  const {state, saveState, saveCreds} = await useMultiFileAuthState(`${global.authFile}`)
    ...
    if (!state.creds.registered) {
      const loginMethod = await question(
        chalk.yellow('\n╔══════════════════════════════╗\n') +
        chalk.yellow('║   اختر طريقة تسجيل الدخول   ║\n') +
        chalk.yellow('╠══════════════════════════════╣\n') +
        chalk.green( '║  1 - رمز الإقران (Code)      ║\n') +
        chalk.cyan(  '║  2 - QR Code                 ║\n') +
        chalk.yellow('╚══════════════════════════════╝\n') +
        chalk.white( 'اختيارك (1 أو 2): ')
      )
      useQR = loginMethod.trim() === '2'
    }
    // ========================================
    ...
    const connectionOptions = {
        ...
        mobile: useMobile,
        browser: useQR ? Browsers.ubuntu('Chrome') : ['Mac OS', 'chrome', '121.0.6167.159'],
        auth: {
         creds: state.creds,
...
if (!conn.authState.creds.registered) {
  if (useQR) {
    console.log(chalk.green('\n📱 امسح الـ QR Code من واتساب > الأجهزة المرتبطة\n'))
  } else {
    let phoneNumber = await question(chalk.blue('\n📞 أدخل رقم الواتساب مع كود الدولة (مثال: 966XXXXXXXXX):\n> '))
    ...
    if (conn.requestPairingCode) {
      ...
      console.log(chalk.green('\n╔══════════════════════╗'))
      console.log(chalk.green('║    رمز الإقران:       ║'))
      console.log(chalk.cyan(`║   ${code.padEnd(19)}║`))
      console.log(chalk.green('╚══════════════════════╝'))
      console.log(chalk.yellow('⏳ أدخله في: واتساب > الأجهزة المرتبطة > ربط جهاز > ربط بالرقم\n'))
    }
  }
}
